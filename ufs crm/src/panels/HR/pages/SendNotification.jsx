...
function HRSendNotification() {
  ...
  const [form, setForm] = useState({ title: '', body: '', mode: 'now', scheduled_at: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.mode === 'now') {
        const result = await sendNow({ title: form.title, body: form.body });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">Send a push notification to all workers</p>
      </div>
