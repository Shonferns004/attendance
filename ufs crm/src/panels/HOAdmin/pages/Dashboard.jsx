...
function HOAdminDashboard() {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getDashboard('hoadmin').then(setData).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-headline-lg font-bold text-gray-900">NGO Dashboard</h1>
        <p className="text-gray-500 mt-1">Overview of your NGO</p>
      </div>
      <div className="grid grid-cols-4 gap-6">
        {[
          { label: 'Total Workers', value: stats.totalWorkers || 0, icon: 'group', color: 'bg-blue-50 text-blue-600' },
          { label: 'HR', value: stats.hr || 0, icon: 'badge', color: 'bg-emerald-50 text-emerald-600' },
          { label: 'Accounts', value: stats.accounts || 0, icon: 'account_balance', color: 'bg-amber-50 text-amber-600' },
          { label: 'Leads', value: stats.leads || 0, icon: 'fiber_new', color: 'bg-rose-50 text-rose-600' },
        ].map((s) => (
...
export default HOAdminDashboard;
