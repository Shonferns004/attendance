...
import { Grid, Spark, Funnel, Users, Brief, Cal2, Heart, LogOut } from './icons';
...
function LoginScreen({ onLogin }) {
  ...
  const { login } = useRec();
  const submit = async () => {
    if (!lid.trim() || !pass.trim()) return;
    setBusy(true); setErr('');
    try {
      await login(lid.trim(), pass);
      onLogin();
    ...
    <div className="login-page">
      <div className="login-card">
        <div className="login-header">
          ...
          <h2>TalentForge</h2>
          <p>Recruiter Studio</p>
        </div>
        <div className="login-form">
          <label className="field">Login ID
            <input value={lid} onChange={e=>setLid(e.target.value)} placeholder="Enter your login ID" autoFocus />
          </label>
...
export default function App() {
  const [loggedIn, setLoggedIn] = useState(!!localStorage.getItem('rec_token'));
  return (
    <RecProvider>
