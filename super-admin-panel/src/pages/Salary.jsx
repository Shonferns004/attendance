    ...
    return (
      <div className="sa-page">
        <div className="sa-page-header">
          ...
          <h3 style={{margin:'8px 0 0'}}>{w?.name || `Worker #${workerId}`} — Salary Detail</h3>
        </div>

        <div className="sa-card">
          <div className="sa-stat-grid" style={{gridTemplateColumns:'repeat(auto-fit, minmax(140px, 1fr))'}}>
            <div className="sa-stat-card"><div className="sa-stat-label">Base Salary</div><div className="sa-stat-value">{formatMoney(detail.salary)}</div></div>
            ...
            {summary.map(s => {
              ...
              return (
                <tr key={s.worker_id}>
                  ...
                  <td>{formatMoney((s.salary || 0) - (s.totalPaid || 0))}</td>
                  <td><span className={`sa-badge ${(s.salary || 0) <= (s.totalPaid || 0) ? 'active' : 'inactive'}`}>
                    {(s.salary || 0) <= (s.totalPaid || 0) ? 'Paid' : 'Pending'}
                  </span></td>
                  <td><button className="btn btn-sm" onClick={() => viewDetail(s.worker_id)}>View</button></td>
                </tr>
              )
            })}
